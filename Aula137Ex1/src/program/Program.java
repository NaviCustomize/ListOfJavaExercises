package program;

import java.util.ArrayList;
import java.util.List;

import account.Account;
import account.BusinessAccount;
import account.SavingAccount;

public class Program {
	
	public static void main(String[]args) {
		
		//Classe abstrato sao classes que nao podem ser instanciada
		
		/*	É uma forma de garantir heranca total, somente sublasses
		 *  nao abstratas podem ser instanciadas,mas nunca a superclasse
		 *  abstrata
		 */
		
		List<Account> list = new  ArrayList<>();
		
		list.add(new SavingAccount(1001,"Alex",500.00,0.01));
		list.add(new BusinessAccount(1002,"Maria",1000.00,400.00));
		list.add(new SavingAccount(1004,"Bob",300.0,0.01));
		list.add(new BusinessAccount(1005,"Anna",500.0,500.0));
		
		double sum =0.0;
		
		for(Account acc :list) {
			sum+=acc.getBalance();
		}
		
		System.out.printf("Total balance: %.2f%n",sum);
		
		for (Account acc : list) {
			acc.deposit(10.0);
		}
		
		for (Account acc : list) {
			System.out.printf("Updated balance for account %d: %.2f%n",acc.getNumber(),acc.getBalance());
		}
	}
	
}
